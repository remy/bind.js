...
describe('arrays', function () {
  ...
  it('should trigger change when array length changes', function (done) {
    ...
    var callback = sinon.spy(function (array) {
      assert.ok(
        ...
        'expected ' + expected + ', is: ' + array.length
      );
    });
  ...
  it('should callback on individual item change', function () {
    ...
    assert.ok(
      data.cats[0].name === 'ninja',
      'first name is ' + data.cats[0].name
    );
  ...
  it('should bubble change to array callback when individual array prop is changed', function () {
    ...
    assert.ok(
      callbacks.cats.callCount == count + 1,
      'callback was called ' + callbacks.cats.callCount
    );
  });
  ...
  it('should support every array method', function (done) {
    ...
    setTimeout(function () {
      assert.ok(
        spy.callCount === count + 0,
        'start count: ' + count + ' vs ' + spy.callCount
      );
      assert.ok(data.cats.pop() === 'missy');
      assert.ok(
        spy.callCount === count + 1,
        'pop count: ' + count + ' vs ' + spy.callCount
      );
      assert.ok(data.cats.shift() === 'ninja');
      assert.ok(
        spy.callCount === count + 2,
        'shift count: ' + count + ' vs ' + spy.callCount
      );
      assert.ok(data.cats.unshift('nap') === data.cats.length);
      assert.ok(
        spy.callCount === count + 3,
        'unshift count: ' + count + ' vs ' + spy.callCount
      );
  ...
  it('should support being entirely replaced', function () {
    ...
    assert.ok(
      count + 1 === single.callCount,
      'call count: ' + single.callCount
    );
    data.cats[0] = 'prince';
    assert.ok(
      single.calledWith('prince'),
      'called with prince: ' + single.args[1]
    );
  });
});
