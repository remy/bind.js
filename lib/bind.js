...
var Bind = (function Bind(global) {
  ...
  function extend(target, object, settings, _path, parent) {
    ...
    forEach(Object.getOwnPropertyNames(object), function eachProp(key) {
      ...
      callback = function valueChangeCallback() {
        if (innerCallback) {
          innerCallback.apply(settings.instance, arguments);
        } else if (parent && parent.__callback && !target.__dirty) {
          parent.__callback.apply(settings.instance);
        }
      };
      ...
      var definition = {
        ...
        set: function (v) {
          ...
          if (settings.ready && typeof v === o && v !== null && !isArray(v) && !v.__callback) {
            value = extend(target[key] || {}, v, settings, path, target[key]);
          } else if (isArray(v)) {
            value = extend(new AugmentedArray(callback, settings), v, settings, path, tmp);
          } else {
            value = v;
          }
          ...
          var cbwrapper = callback.bind(target, value, old);

          if (target[key] instanceof Object && !target[key].__callback) {
            target[key].__callback = cbwrapper;
          }
          ...
          if (settings.ready) {
            ...
            if (callback) {
              cbwrapper();
            } else if (parent && parent.__callback) {
      ...
      if (typeof value === o && value !== null && !isArray(value)) {
        target[key] = extend(target[key] || {}, value, settings, path, target[key]);
      } else if (isArray(value)) {
        var tmp = {}; // required to capture the new callback
        target[key] = extend(new AugmentedArray(callback, settings), value, settings, path, tmp);
        tmp.__callback = target[key].__callback;
