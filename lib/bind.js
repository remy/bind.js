...
var Bind = (function Bind(global) {
  ...
  function AugmentedArray(callback) {
    ...
    Object.defineProperty(this, 'length', {
      ...
      enumerable: true,
      set: function (v) {
        // console.log('set called with %s, length: %s, dirty? %s', v, length, this.__dirty, (new Error).stack.split('\n').slice(2,3).pop().replace(/^.*as (\w+)\b.*$/, 'via `$1`'));
        if (this.__dirty) {
          length = v;
      ...
      var definition = {
        ...
        set: function (v) {
          ...
          var cbwrapper = function initValue(callback, value, old, target, parentIsArray) {
            if (value instanceof AugmentedArray) {
              return;
            }
            callback(value, old);
            // debugger;
